...
class ChiTietDonDatPhongScreen extends StatefulWidget {
  ...
  @override
  State<ChiTietDonDatPhongScreen> createState() => _ChiTietDonDatPhongScreenState();
}
...
class _ChiTietDonDatPhongScreenState extends State<ChiTietDonDatPhongScreen> {
  ...
  final BookingController _bookingController = BookingController(FirebaseBookingService());
  final ServiceDetailController _serviceDetailController = ServiceDetailController();
  ...
  Future<void> _loadDetails() async {
    try {
      ...
      if (_selectedStatus == 'Hoàn tất' && bookingId != null && bookingId.isNotEmpty) {
  ...
  Future<void> _sendStatusNotificationToCustomer({
    ...
    // Local push (in-app) notification
    if (status == 'Xác nhận') {
      BookingNotificationHelper.notifyBookingSuccess(bookingId);
    } else if (status == 'Hủy') {
      BookingNotificationHelper.notifyCancelSuccess(bookingId);
    } else if (status == 'Đã nhận phòng') {
      BookingNotificationHelper.notifyCheckIn(bookingId);
    } else if (status == 'Hoàn tất') {
      BookingNotificationHelper.notifyComplete(bookingId);
    }
  }
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildInfoRow(
              'Email:',
              _customer?.email ?? 'Không có dữ liệu',
            ),
            _buildInfoRow(
              'Số điện thoại:',
            ...
            _buildInfoRow('Số lượng phòng:', widget.booking.quantity.toString()),
            _buildInfoRow('Số lượng khách:', widget.booking.guests.toString()),
            _buildInfoRow('Ngày nhận phòng:', _formatDate(widget.booking.checkIn)),
            _buildInfoRow('Ngày trả phòng:', _formatDate(widget.booking.checkout)),
            Row(
              children: [
                  ...
                  _services.isEmpty
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final quantityController = _serviceQuantityControllers[serviceId];
                                    ...
                                    onChanged: (value) {
                                      setState(() {
                                        if (value == true) {
                                          ...
                                          quantityController?.text = quantityController.text.isEmpty
                                              ? '1'
            ...
            Center(
              child: ElevatedButton(
                onPressed: (_initialStatus == 'Hủy' || _initialStatus == 'Hoàn tất' || _initialStatus == 'Không nhận phòng' || _isSaving)
                    ? null
                    : () async {
                        final bookingId = widget.booking.id;
                        if (bookingId == null || bookingId.isEmpty) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text('Không tìm thấy mã đơn để cập nhật.'),
                            ),
                          );
                          return;
                        }
                        ...
                        try {
                          if (_selectedStatus == 'Hoàn tất' && _selectedServiceIds.isNotEmpty) {
